...
func NewPush(ctx context.Context, lng ipld.NodeGetter, mfst *dag.Manifest, remote Remote) (*Push, error) {
...
// Push coordinates requesting & sending a manifest to a remote, tracking progress and state
type Push struct {
...
func (snd *Push) Do() (err error) {
	...
	// responses
	snd.sid, snd.diff, err = snd.remote.PushStart(snd.mfst)
	if err != nil {
		return err
	}
...
func (snd *Push) doActualPush() (err error) {
	...
	for {
		...
		case hash := <-s.blocksCh:
			...
			go func() {
				...
				s.responses <- s.remote.PushBlock(s.sid, hash, node.RawData())
			}()
		case <-s.stopCh:
			return
		case <-s.ctx.Done():
