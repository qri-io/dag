...
type subDAGGenerator struct {
  ...
  Sizes              []uint64
  // PrevLabels is the map of labels from the original dag Info
...
func (s *subDAGGenerator) convert() (*Info, error){
  ...
  if s.RootIndex < 0 || s.RootIndex > len(s.PrevManifest.Nodes)-1 {
...
// SubInfoAtIndex returns a sub-Info, the DAG, sizes, and labels,
// with the given index as root of the DAG
func (i *Info) SubInfoAtIndex(idx int) (*Info, error) {
  return newSubDAGGenerator(i, idx).convert()
}

// SubInfoAtID returns a sub-Info, the DAG, sizes, and labels,
// with the given id as root of the DAG
func (i *Info) SubInfoAtID(id string) (*Info, error) {
  idx, err := i.Manifest.IDIndex(id)
  if err != nil {
    return nil, err
  }
  return i.SubInfoAtIndex(idx)
}

// SubInfoAtLabel returns a sub-Info, the DAG, sizes, and labels,
// with the given label as root of the DAG
func (i *Info) SubInfoAtLabel(label string) (*Info, error) {
